...
# Dual-Attention Mixing Transformer
class DAMTransformer(nn.Module):
    def __init__(self, dim, num_head=4, chsa_head_ratio=0.25, window_size=8, shift=0, head_dim=None, qkv_bias=True, mv_ver=2,
        ...
        super(DAMTransformer, self).__init__()
    ...
    def forward(self, x, sp_=None, ch_=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if sp_ is not None and ch_ is not None:
                sp_ = F.pad(sp_, (0, pad_w, 0, pad_h), mode="constant", value=0)
                ch_ = F.pad(ch_, (0, pad_w, 0, pad_h), mode="constant", value=0)
...
if __name__ == '__main__':
    ...
    block = DAMTransformer(dim=64)

    input = torch.randn(8, 64, 28, 28) # 输入B C H W
    sp_ = torch.randn(8, 48, 28, 28)
    ch_ = torch.randn(8, 16, 28, 28)

    # Forward pass
    output, sp, ch = block(input, sp_, ch_)
