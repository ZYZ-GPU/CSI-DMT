...
class Fusion:
    def __init__(self,
                 ...
                 modelpath='RunTimeData/Model weights/best_network.pth',
    ...
    def FusionProcess(self, model, eval_list_A, eval_list_B, savepath, threshold):
        ...
        with torch.no_grad():
            for A, B in eval_loader_tqdm:
                ...
                NetOut, D = model(A, B)
                # 保存 NetOut 图像
                netout_img = NetOut[0].detach().cpu().clamp(0, 1)  # 确保在[0,1]范围
                save_image(netout_img, f'./Results{savepath}/NetOut_{str(cnt).zfill(2)}.png')
                ...
                # 保存原始 D
                # save_image(D, f'./Results{savepath}/D_before_{str(cnt).zfill(2)}.png')
                D = self.ConsisVerif(D, threshold)
                # 保存经过 ConsisVerif 之后的 D
                save_image(D, f'./Results{savepath}/D_after_{str(cnt).zfill(2)}.png')
                ...
                if self.window_size > 0:
                    ...
                    # 提取标记为 9999 的决策图
                    # decisionmap_9999 = torch.where(decisionmap == 9999., 1., 0.)

                    # 转换为 NumPy 格式
                    # decisionmap_9999_np = einsum('c w h -> w h c', decisionmap_9999).clone().detach().cpu().numpy()

                    # 放大对比度（可选）
                    # decisionmap_9999_np = (decisionmap_9999_np * 255).astype(np.uint8)

                    # 颜色反转
                    # decisionmap_9999_inverted = 255 - decisionmap_9999_np

                    # 保存反转后的决策图
                    # cv2.imwrite('./Results' + savepath + '/' + self.DATASET_NAME + '-' + str(cnt).zfill(
                    #     2) + '_decisionmap_9999_inverted.png',
                    #             decisionmap_9999_np)
